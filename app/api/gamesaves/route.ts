...
export async function POST(request: Request) {
  try {
    ...
    if (!gameId || !userId || !saveName || !data) {
      return NextResponse.json({ message: 'Game ID, User ID, Save Name, and Data are required, womp womp' }, { status: 400 });
    }
    ...
    if (!result.insertedId) {
      return NextResponse.json({ message: 'Womp womp, save fail error' }, { status: 500 });
    }

    return NextResponse.json({ message: 'Yippie! save loading...', _id: result.insertedId.toHexString() }, { status: 201 });
  } catch (error) {
